...
func newCloudDatabaseInstanceStateRefreshFunc(d *schema.ResourceData, meta interface{}) resource.StateRefreshFunc {
	...
	return func() (interface{}, string, error) {
		...
		if taskID == "" {
			return nil, "", fmt.Errorf("[ERROR] Not found task_id of action create instance %s", d.Id())
		}

		// retry
		retry := maxRetry
		err := resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
			resp, err := client.CloudDatabase.Tasks().Get(context.Background(), taskID)

			if err != nil {
				retry -= 1
				if retry > 0 {
					time.Sleep(timeSleep)
					return resource.RetryableError(fmt.Errorf("[ERROR] Get information of task [%s] error: %v. Retrying", taskID, err))
				}
				return resource.NonRetryableError(fmt.Errorf("[ERROR] Get information of task [%s] error: %v. Can't retry", taskID, err))
			}

			// if the task is not ready, we need to wait for a moment
			if !resp.Ready {
				return resource.RetryableError(fmt.Errorf("[DEBUG] Task create cloud database instance is not ready"))
			}
			return nil
		})
...
func deleteCloudDatabaseInstanceStateRefreshFunc(d *schema.ResourceData, meta interface{}) resource.StateRefreshFunc {
	...
	return func() (interface{}, string, error) {
		...
		err := resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
			...
			// if the task is not ready, we need to wait for a moment
			if !resp.Ready {
				return resource.RetryableError(fmt.Errorf("[ERROR] [DEBUG] Task delete cloud database instance is not ready"))
			}
			return nil
		})
		...
		return ins, strconv.FormatBool(resp.Ready), nil
	}
}
