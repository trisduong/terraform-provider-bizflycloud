...
func newCloudDatabaseNodeStateRefreshFunc(d *schema.ResourceData, meta interface{}) resource.StateRefreshFunc {
	...
	return func() (interface{}, string, error) {
		...
		if taskID == "" {
			return nil, "", fmt.Errorf("[ERROR] Not found task_id of action create node %s", d.Id())
		}
		...
		err := resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
			...
			if !resp.Ready {
				return resource.RetryableError(fmt.Errorf("[DEBUG] Task create cloud database node is not ready"))
			}
			return nil
		})
...
func deleteCloudDatabaseNodeStateRefreshFunc(d *schema.ResourceData, meta interface{}) resource.StateRefreshFunc {
	...
	return func() (interface{}, string, error) {
		...
		err := resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
			...
			// if the task is not ready, we need to wait for a moment
			if !resp.Ready {
				return resource.RetryableError(fmt.Errorf("[DEBUG] Task delete cloud database node is not ready"))
			}
			return nil
		})
