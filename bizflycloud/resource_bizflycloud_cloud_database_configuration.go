...
func resourceBizFlyCloudCloudDatabaseConfigurationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	cfc := &gobizfly.CloudDatabaseConfigurationCreate{
		...
		ConfigurationParameters: readArrayParameters(d.Get("configuration_parameters").(map[string]interface{})),
	...
	return resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
		...
		if err != nil {
			...
			if retry > 0 {
				...
				return resource.RetryableError(fmt.Errorf("[ERROR] create cloud database configuration %s failed: %s. Retrying", d.Get("configuration_name"), err))
			}

			return resource.NonRetryableError(fmt.Errorf("[ERROR] create cloud database configuration %s failed: %s. Can't retry", d.Get("configuration_name"), err))
		}
		...
		if err != nil {
			return resource.NonRetryableError(fmt.Errorf("[ERROR] create cloud database configuration (%s) failed: %s. Can't retry", d.Get("configuration_name").(string), err))
		}
...
func waitForCloudDatabaseConfigurationCreate(d *schema.ResourceData, meta interface{}) (interface{}, error) {
	log.Printf("[INFO] Waiting for cloud database instance (%s) to be ready", d.Get("configuration_name").(string))
