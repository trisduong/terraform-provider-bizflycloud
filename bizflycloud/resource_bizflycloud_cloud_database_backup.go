...
func resourceBizFlyCloudCloudDatabaseBackupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("instance_id").(string) != "" {
		...
		return fmt.Errorf("[ERROR] create cloud database backup %s failed: not found resource to backup", d.Get("backup_name"))
	}
	...
	return resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
		...
		if err != nil {
			...
			if retry > 0 {
				...
				return resource.RetryableError(fmt.Errorf("[ERROR] create cloud database backup %s failed: %s. Retrying", d.Get("backup_name"), err))
			}

			return resource.NonRetryableError(fmt.Errorf("[ERROR] create cloud database backup %s failed: %s. Can't retry", d.Get("backup_name"), err))
		}
		...
		if err != nil {
			return resource.NonRetryableError(fmt.Errorf("[ERROR] create cloud database backup (%s) failed: %s. Can't retry", d.Get("backup_name").(string), err))
		}

		err = resourceBizFlyCloudCloudDatabaseBackupRead(d, meta)
		if err != nil {
			return resource.NonRetryableError(fmt.Errorf("[ERROR] read cloud database backup (%s) failed: %s. Can't retry", d.Get("backup_name").(string), err))
		}
...
func waitForCloudDatabaseBackupCreate(d *schema.ResourceData, meta interface{}) (interface{}, error) {
	log.Printf("[INFO] Waiting for cloud database instance (%s) to be ready", d.Get("backup_name").(string))
