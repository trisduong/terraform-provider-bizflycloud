	...
	return resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
		...
		if err != nil {
			...
			if retry > 0 {
				...
				return resource.RetryableError(fmt.Errorf("[ERROR] create cloud database schedule %s failed: %s. Retrying", d.Get("schedule_name"), err))
			}

			return resource.NonRetryableError(fmt.Errorf("[ERROR] create cloud database schedule %s failed: %s. Can't retry", d.Get("schedule_name"), err))
		}
		...
		if err != nil {
			return resource.NonRetryableError(fmt.Errorf("[ERROR] create cloud database schedule (%s) failed: %s. Can't retry", d.Get("schedule_name").(string), err))
		}
